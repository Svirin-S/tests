...
def get_all_doc_owners_names(documents_):
    users_list = []
    for current_document in documents_:
        try:
...
def show_document_info(document_):
    documents_list = []
    for current_document in document_:
        doc_type = current_document['type']
        doc_number = current_document['number']
        doc_owner_name = current_document['name']
        documents_list.append('{} "{}" "{}"'.format(doc_type, doc_number, doc_owner_name))
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            print(show_document_info(documents))
        elif user_command == 's':
            ...
            print(add_new_doc(new_doc_number, new_doc_type, new_doc_owner_name, new_doc_shelf_number))
            
        elif user_command == 'd':
            user_doc_number = input('Введите номер документа - ')
            print(delete_doc(user_doc_number))
        elif user_command == 'm':
            move_doc_to_shelf()
